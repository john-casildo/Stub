...
**School-assignment load-testing tooling** (`server/seed/`, `server/loadtest/`) is separate from both backends above — it's grading/demo tooling that bulk-seeds Postgres directly and load-tests the custom `server/` API with k6, visualized live via Prometheus + Grafana (added to `server/docker-compose.yml`). It has no automated test coverage (not app logic) and is meant to be run against a disposable local database — see those two file-map rows above for how to run it. **Real-run finding (2026-09-16): the full ~500,000-request/7-minute profile has not been achieved against this local Docker stack, and the root cause is out of this task's scope to fix.** Two full 7-minute runs were executed against a freshly seeded 1.2M-transaction database: the first (original `scenario.js`, `preAllocatedVUs: 300`/`maxVUs: 1000`) completed only 110,766 of the ~493,020 target requests with 0% failures (383,273 dropped — k6's own VU pool couldn't sustain the target arrival rate given this API's real per-request latency under load, avg 3.12s/p95 6.49s, itself a symptom of `server/src/db.ts`'s `pg.Pool` defaulting to `max: 10` concurrent connections); the second (VUs raised to `2000`/`8000` to remove that ceiling) completed 128,580 requests but with 93.28% *failing* (timeouts/connection resets), and `docker compose ps` confirmed the `api` container had crashed and been auto-restarted mid-run. The crash's cause was found in `server/src/db.ts`: pooled `pg.Client` connections have no `.on('error', ...)` handler, so Node's default EventEmitter behavior turns a dropped/reset DB connection (expected under this much connection churn) into an uncaught exception that kills the whole process (confirmed via `docker compose logs api`: `Error: Connection terminated unexpectedly` immediately followed by a full process exit and restart). Fixing this — a larger/tuned connection pool plus a real error handler on it, and possibly clustering the single Node process across the host's CPU cores — requires changes inside `server/src/**`, which this load-testing task's plan explicitly does not authorize touching. The seeder and k6 script themselves both work correctly (100% checks pass on `SMOKE=1`, and every non-timed-out request in both full runs got a correct response) — this is a real backend capacity/robustness gap, not a bug in the load-testing tooling. Treat the ~145,000-peak-minute/~493,020-total figures in the file-map rows above as this tooling's *target design profile*, not a number this specific local environment has been shown to sustain; a follow-up task against `server/src/**` is needed before that target is achievable here.
