...
| `app/lib/screens/edit_entry_screen.dart` | `EditEntryScreen` — correct/review transaction details (merchant, amount, category, date, source); merchant and amount are genuinely editable (each opens an `_EditFieldDialog` text-entry dialog — fixed a pre-existing bug where the fields looked editable via `StubFieldRow(editable: true)` but had no `onTap` wired to change them) and saves/deletes via `TransactionRepository` through `RootShell`; `isCreating` (default `false`) hides the Delete row for the scan/manual-entry creation flow, where there's nothing yet to delete; DATE and SOURCE are both read-only rows (`dateLabel`/`sourceLabel`, from `transaction.dart`'s shared label helpers) — fixed a real bug where SOURCE was wired to `transaction.dateLabel` instead of the actual source, silently showing a "M/D"-shaped date (e.g. "6/9") where "Manual"/"Receipt scan"/etc. belonged; still no camera/re-capture feature; `categories` takes full `Category` objects (not names) so the AMOUNT row's currency symbol (`formatCurrency(_amount, currencyCode: _selectedCurrencyCode)`) re-resolves against the currently-selected category's own `currencyCode` override every time a different CATEGORY chip is tapped, rather than always showing the app's global default symbol |
| `app/lib/screens/manual_entry_screen.dart` | `ManualEntryScreen` — manual transaction entry form; now reachable from `LedgerScreen`'s FAB (see Open items — the "no UI trigger" gap is resolved) and writes via `TransactionRepository`; its amount field shares `AddCategoryScreen`'s live comma-formatting and the same `_maxAmount` (10,000,000,000) sanity cap via `ThousandsSeparatorInputFormatter(maxValue: _maxAmount)`, blocking a keystroke that would exceed it live (the on-save snackbar check remains as defense-in-depth for a value arriving another way, e.g. a pre-filled `initialAmount`); a `FocusNode` (`_amountFocusNode`) selects the field's entire text the moment it gains focus, so tapping it and typing immediately replaces "0.00" (or a pre-filled amount) instead of requiring the user to erase it first; also takes optional `initialAmount`/`initialMerchant` (`double?`/`String?`, both null on every entry point except a Siri Shortcuts quick-log deep link — see `RootShell`'s entry above) that pre-fill `_amountController`/`_merchantController` on construction; `categories` takes full `Category` objects (not names) — the hero amount field's `prefixText` is `currencySymbolFor(_selected.currencyCode)` (`util/currency.dart`), so tapping a different CATEGORY chip immediately swaps the leading symbol (e.g. `$` → `₡`) to match that category's own currency override, same rule `EditEntryScreen`'s AMOUNT row follows |
| `app/lib/util/thousands_input_formatter.dart` | `ThousandsSeparatorInputFormatter` — a `TextInputFormatter` that live-inserts thousands commas into a numeric field as you type ("1000000" → "1,000,000", preserving cursor position and a single decimal point); callers strip the commas back out (`.replaceAll(',', '')`) before parsing. Takes an optional `maxValue` — a keystroke that would push the parsed value above it is rejected outright (field just doesn't change) rather than only catching it at save time; `AddCategoryScreen`'s limit field and `ManualEntryScreen`'s amount field both pass their own `_maxLimitAmount`/`_maxAmount` (same 10,000,000,000 value) here, so typing past the cap is blocked live in both places, not just on Save |
...
| `analytics/README.md` | The assignment-facing writeup of this whole subproject (architecture diagram, the 10 file formats, how to run everything from scratch) — read this first for anything analytics-related; this file-map section only covers what's non-obvious/historical |
| `analytics/department-files/generate.ts` | School-assignment Faker generator for **ten** synthetic "shadow IT" department files, each in a genuinely different format and each with its own mismatched/differently-cased employee-name field (see `analytics/README.md`'s table for the full list — Excel/CSV/JSON/pipe-text/YAML/XML/fixed-width-text/INI/Markdown/a custom log format). All ten draw from one **fixed pool of 20 employees** (`EMPLOYEES`, generated once) rather than a fresh random name per row — without this, "spend by employee"-style aggregations are meaningless, since a random name essentially never repeats across a few hundred rows. Several files are **dirty on purpose** (extra whitespace, inconsistent casing, mixed date formats, real nulls) so the ETL has real cleaning work to do, not just a rename. After writing all ten locally, uploads them to the data lake (see the `minio`/data-lake row below) — the local `department-files/output/` copy is just a convenience for inspection, not what the pipeline actually reads. Regenerate via `cd analytics && npm run generate-files` |
| `analytics/pipeline/` | School-assignment nightly analytics pipeline (`clone.ts` + `etl.ts` + `index.ts`, Dockerized) — clones the real `server/` production Postgres into a separate Analytics DB via `pg_dump \| psql` (`clone.ts`), then downloads the ten department files from the data lake and normalizes each one's mismatched employee-name field into one `nombre_empleado` column across **ten** new tables (`accounting_records`/`department_budgets`/`ops_metrics`/`hr_records`/`marketing_campaigns`/`legal_contracts`/`purchase_records`/`inventory_items`/`support_tickets`/`audit_log`, `etl.ts`). `index.ts` runs both via `node-cron` at midnight by default, or once immediately via `RUN_NOW=1`. Managed by `analytics/terraform/` (below), not `server/docker-compose.yml`. `clone.ts` has been through three real, confirmed bug fixes in the same failure family, all found during Task 8's clean-slate verification and its review: (1) it waits on **both** the `pg_dump` and `psql` child processes' exit codes before deciding success/failure — the original version only checked `psql`'s, so a `pg_dump` failure (a Postgres client/server version mismatch — see the Dockerfile row below) went completely silent, since `psql` still exits `0` having received an empty/truncated stdin stream and restored nothing (a clone that "succeeds" in under a second, instead of the ~14-16s a real 1.2M-row `pg_dump \| psql` takes, is the tell that this regressed); (2) `psql` runs with `-v ON_ERROR_STOP=1` — without it, a failed statement mid-restore (e.g. a broken `COPY` block) still leaves `psql` exiting `0`, so the clone would report success with partial/corrupt data even with fix #1 in place; (3) `restore.stdin.on('error', () => {})` absorbs the EPIPE that fires when `psql` dies early (e.g. an unreachable target) and `dump.stdout`'s pipe tries to write into its now-closed stdin — **but that absorb-and-ignore alone reintroduces a worse bug**: with nothing left draining `dump.stdout`, `pg_dump` blocks forever on its own `write()` once the OS pipe buffer fills, hanging the whole clone indefinitely instead of failing (reproduced live during Task 8's own re-verification: three real `pg_dump` processes were left permanently running in the container, invisible from outside since `docker exec`'s own SIGTERM doesn't reach the exec'd process — `docker restart analytics-pipeline` was needed to clear them). The real fix is in `restore.on('close', ...)`: if `psql` closes before `pg_dump` does, `dump.kill()` is called so a dead restore can't leave the dump side hanging. Verified live against an unreachable `ANALYTICS_DATABASE_URL` target: rejects cleanly in seconds with the real `psql: error: connection ... Connection refused` message, exit code 1, and zero stray processes left behind (`docker top` confirmed clean) |
...
| `analytics/terraform/` | Local Terraform (`kreuzwerker/docker` provider) managing four containers, all joined to `server/docker-compose.yml`'s existing `server_default` network via a `data "docker_network"` lookup (never creating or destroying it), none touching `server/`'s own stack: **`analytics-db`** (a second, independent Postgres 16 on host port 5433), **`analytics-datalake`** (MinIO, S3-compatible object store for the ten department files — see `department-files/generate.ts`'s row — console at `:9001`, S3 API at `:9000`, credentials `minioadmin`/`minioadmin123`, image is `quay.io/minio/minio` since MinIO discontinued its `minio/minio` Docker Hub repo), **`analytics-pipeline`** (built from `analytics/pipeline/Dockerfile`), and **`analytics-metabase`** (a BI dashboard layer added on request after the original assignment, matching the instructor's reference diagram's PowerBI/Tableau/Metabase box — its own small app database lives in a named volume, `:3002`; connect it to `analytics-db` via its first-run setup wizard, not via Terraform). `terraform apply`/`terraform destroy` from this directory. **A `terraform destroy` on a machine where `server-postgres-1` (or anything else) is also running `postgres:16` will reliably fail to remove `docker_image.analytics_postgres`** — Docker refuses to delete an image layer another, unrelated container still references (confirmed twice: once during Task 7's own verification, once again during Task 8's independent clean-slate re-run) — while the containers and volume it actually owns still get destroyed cleanly. The fix is `terraform state rm docker_image.analytics_postgres` (not `docker rmi -f`, which would risk disrupting the unrelated container); the next `terraform apply` recreates a correctly-tracked image resource with no other side effects. `docker_image.pipeline` now carries a `triggers` map (`filesha256`/`fileset`-hashed over the Dockerfile, every file under `pipeline/src/`, and `package.json`) — without it the `kreuzwerker/docker` provider keys this resource on name/image_id, not build-context content, so `terraform apply` silently reported "No changes" even after real source edits (verified live during the final review: touching `index.ts` alone made `terraform plan` correctly show `docker_image.pipeline`/`docker_container.pipeline` as needing replacement). A pipeline source change now just needs a normal `terraform apply` — no manual rebuild step required. The pipeline container no longer bind-mounts `department-files/output/` (removed once the ETL started fetching from the data lake instead — the mount was read-only, which broke once `etl.ts` needed to write the downloaded files somewhere) |
...
6. *(further post-merge exploration: two more real runs against the reverted single-process config + the new index, on a freshly reseeded database)* — the first of these two produced an anomalous, misleading 138,571-request/85.03% "success" number that turned out to be a false signal: running `npm test` in between seeding and this run **truncated the seeded data** (Jest's `beforeEach` hooks share the same database as the seeder and wipe all four tables for test isolation — see `server/test/*.test.ts`'s file-map entry). With almost no real data left, `GET /transactions`/`/budgets`/`/categories` kept returning fast, trivial *empty* result sets (still 200 OK, inflating the apparent success rate), while `GET /account` and `POST /transactions` — which do exact-id lookups — failed 100% (`404 user_not_found`/`category_not_found`) against ids that no longer existed. **Real operational gotcha, worth remembering: don't run `npm run seed` and `npm test` against the same Postgres instance without reseeding again afterward** — the two are not isolated from each other. After reseeding properly and confirming a token round-trips a real `200` first, the next real 7-minute run against the same config as run 4 produced 81,479 total requests (80.95% success, 19.04% failed, avg `http_req_duration` 9.38s) — consistent with runs 3/4's ~70-80k range, confirming the ceiling is stable and reproducible once the data-integrity issue is ruled out.
...
**Follow-up review pass, same day**: review reproduced two more real bugs
in the same failure family as the two above, both in `clone.ts` —
`psql` running without `-v ON_ERROR_STOP=1` (a failed statement mid-restore
would still leave `psql` exiting `0`, reporting success on a partial/
corrupt restore) and an unhandled EPIPE crash when `psql` dies early
(`dump.stdout`'s pipe writing into `psql`'s already-closed stdin). Fixed
both — but verifying the second fix live surfaced a **third**, worse bug
the literal one-line fix introduced: silently absorbing that EPIPE without
stopping `pg_dump` left `pg_dump` blocked forever on its own `write()`
once nothing was left draining its stdout, hanging the whole clone
indefinitely instead of failing. Reproduced for real against an
unreachable `ANALYTICS_DATABASE_URL` — three `pg_dump` processes were
left permanently running inside the container, invisible from outside
since `docker exec`'s SIGTERM doesn't reach the exec'd process;
`docker restart analytics-pipeline` was needed to clear them. Fixed by
killing `pg_dump` when `psql` closes first (see `clone.ts`'s file-map
entry above for the full detail). Re-verified after all three fixes,
rebuilt into the image: (a) an unreachable target now rejects cleanly in
seconds with the real `psql: error: connection ... Connection refused`
message and zero stray processes (`docker top` confirmed clean), and (b)
a real full pipeline run against the actual infrastructure still produced
the exact same numbers as above (3000/17973/1,200,000 clone match,
120/36/80 ETL rows, same real normalized names) in ~16s.

**Final whole-branch review fix round (2026-09-21)**: four more findings
fixed and verified live. `analytics/.dockerignore` now excludes
`terraform/` — without it, `COPY . .` in the Dockerfile pulled the entire
`terraform/` directory (including `terraform.tfstate`'s plaintext
production DB connection string and a 63MB `.terraform/` provider cache)
into the image build context, since the build context is `analytics/`,
not `pipeline/`. `docker_image.pipeline`'s new `triggers` (see
`analytics/terraform/`'s file-map entry above) were exercised for real,
not just written and trusted: `clone.ts`/`index.ts` edits during this same
round caused `terraform plan` to correctly show the image/container as
needing replacement each time, confirming the fix works. `clone.ts` now
has a `withConnectTimeout(url, seconds)` helper appending a
`connect_timeout=10` query param to both `sourceUrl`/`targetUrl` before
spawning `pg_dump`/`psql` — closes the mirror case of the already-fixed
EPIPE deadlock (that one was "psql dies early, pg_dump hangs on write";
this one is "pg_dump/psql itself hangs on connect" against an
unreachable-but-routable host) — verified the resulting URL shape is one
`psql` actually accepts by running it live against the real
`analytics-db` container. `index.ts`'s cron callback now has a
module-level `running` guard so an overlapping tick (e.g. one arriving
while a prior run is hung on the connect-timeout case above) is skipped
with a logged message instead of spawning a second concurrent
`pg_dump`/`psql` pair. **The cron-tick-firing path itself (as opposed to
`RUN_NOW=1`) was also verified live for the first time this round**: the
cron expression was temporarily changed to `*/2 * * * *`, the image
rebuilt via `terraform apply` (itself a live proof the triggers fix
works — touching `index.ts` did cause a rebuild), and the container run
in its real default (non-`RUN_NOW`) mode; `docker logs` showed two real
ticks fire on their own two minutes apart (21:04:00 and 21:06:00 UTC),
each producing the full real `Starting nightly analytics pipeline` /
`Clone complete.` / `ETL complete: ...` / `Pipeline complete` log
sequence with no manual `docker exec` involved. The cron expression was
then reverted to `0 0 * * *` and the image rebuilt again; the running
container's compiled `dist/pipeline/src/index.js` was confirmed to say
`0 0 * * *` with zero remaining occurrences of the test `*/2` pattern. A
final `RUN_NOW=1` regression run after all fixes reproduced the same
3000/17973/1,200,000 clone match and 120/36/80 ETL row counts as every
prior verification.
