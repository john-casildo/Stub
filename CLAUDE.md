...
| `server/loadtest/scenario-15min.js` | Exploratory, non-deliverable variant of `scenario.js` — same ramp shape, peak held for 9 minutes instead of 1 (15 minutes total), built to see whether a longer window changes the throughput ceiling finding. It doesn't: one real run produced 152,492 total requests (86.42% success, 13.57% failed), scaling sensibly with the 7-minute runs' numbers rather than revealing a different ceiling. Run via `k6 run --out experimental-prometheus-rw scenario-15min.js` from `server/loadtest/`, same prerequisites as `scenario.js` |
...
7. *(exploratory 15-minute variant, `server/loadtest/scenario-15min.js` — same ramp shape as the assignment's `scenario.js` but the peak held for 9 minutes instead of 1, not the assignment deliverable)* — 152,492 total requests, 86.42% success / 13.57% failed, avg `http_req_duration` 27.7s, 1,356,687 dropped iterations. Scales sensibly with the 7-minute runs (roughly double the total over roughly double the duration), reinforcing the same conclusion: this is a stable, reproducible per-second capacity ceiling, not something a longer run window overcomes.
