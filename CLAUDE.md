...
| `app/lib/data/fakes.dart` | `FakeCategoryRepository`, `FakeTransactionRepository`, `FakeBudgetRepository`, `FakeTextRecognitionService` — in-memory test doubles. `FakeBudgetRepository` takes an *optional* `FakeCategoryRepository` link: when given, it resolves real category names on `create()` and drops budgets from `list()` once their category is gone (mirroring the schema's `ON DELETE CASCADE`); without it, falls back to a placeholder name and no cascade — a deliberate, non-naive design, not a plain in-memory store. `FakeTextRecognitionService` just returns a configured `List<RecognizedLine>` from `recognizeText` — no OCR |
...
| `app/lib/screens/scan_screen.dart` | `ScanScreen` — real capture flow: pick a source (`image_picker`, camera or photo library; a picker/permission failure or a cancelled pick shows a snackbar and calls `onClose`), pick an image type (Receipt/Payment app/Bank screenshot via `StubChip`, mapped to `TransactionSource`), then runs `TextRecognitionService.recognizeText` + `parseReceiptLines` (any OCR failure is caught and falls back to an all-null `ParsedReceipt` rather than crashing) and calls `onScanned(parsed, source)` — no longer a hardcoded confirm-card. `debugInitialImagePath` is a test-only seam (real `image_picker` results can't be faked in a widget test) that skips straight to the type-picker stage with a given path so the OCR/parse/`onScanned` wiring past image capture is still testable |
...
- **Testing gap**: all 104 tests are unit/widget tests against in-memory
